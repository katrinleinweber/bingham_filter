...
classdef BinghamDistribution < AbstractHypersphericalDistribution
    ...
    methods (Static)                
        ...
        function B = fit_without_weights(samples, options)
            % Fits Bingham parameters to a set of samples
            % Parameters:
            %   samples (d x n matrix)
            %       matrix that contains one sample per column
        ...
        function x = surface_area_hypersphere(d)
            ...
            elseif d==3
               ...
               x = (2*pi/(d-1))*BinghamDistribution.surface_area_hypersphere(d-2);
            end
        end
        
        % glover's bingham.c : bingham_cluster() and bingham_fit_mlesac()
        function [Bingham_arr, Bingham_weights] = cluster_fit(samples, min_points, options)
%             min_points = 10000;
            num_iters = 100;
            num_outliers = 0;
            dim_samples = size(samples, 1);
            p0 = 1/BinghamDistribution.surface_area_hypersphere(dim_samples-1)
            samples_left = samples; % copy
            num_samples_left = size(samples_left,2);
  
            for i=1:num_iters
                B_curr = BinghamDistribution.fit_without_weights(samples_left, options);
                prob_samples = B_curr.pdf(samples_left);
                outlier_mask = not(prob_samples > p0); % glover/bingham.c/bingham_fit_mlesac
                num_outliers = sum(outlier_mask);
                Bingham_weights(i) = num_samples_left - num_outliers;

                if (Bingham_weights(i) >= min_points)
                    Bingham_arr(i) = B_curr;
                else
                    disp("break : Bingham_weights(i) < min_points")
                    break;
                end
                
                if(num_outliers < min_points)
                    disp("break: num_outliers < min_points")
                    break;
                end

               points_left = num_outliers;
               % let's get the remainder samples by applying the mask.
               % samples is d by n, outliers is 1 by n
               samples_left_curr = [];
               for row_idx=1:dim_samples
                   curr_row = samples_left(row_idx, :);
                   curr_row_masked = curr_row(outlier_mask);
                   samples_left_curr = [samples_left_curr; curr_row_masked];
               end
               samples_left = samples_left_curr;
            end
            
            % normalize weights
            Bingham_weights = Bingham_weights / sum(Bingham_weights); 
        end
